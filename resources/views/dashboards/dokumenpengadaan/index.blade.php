                  ...
                  <div class="modal-body">
                     {{ csrf_field() }}
                      <div class="row mb-3">
                        ...
                        <div class="col-sm-9">
                          <select name="aset_id[]" id="nama_aset" class="form-control select2-add" multiple="multiple" placeholder="pilih aset (bisa multiple)" required>
                            @foreach ($semua_aset as $aset)
                              <option value="{{ $aset['id'] }}">{{ $aset['barang']['nama_barang'] }} - {{ $aset['kode_aset'] }}</option>
                            @endforeach
                          </select>
                        </div>
                      </div>
                      ...
                      <div class="row mb-3">
                        ...
                        <div class="col-sm-9">
                          <input type="file" class="form-control" id="upload_dokumen" name="dokumen_upload" required>
                        </div>
                      </div>
                  </div>
              ...
              @foreach ($semua_dokumen as $dokumen)
                <tr>
                  ...
                  <td>
                    ...
                    <button type="button" id="#previewPdf" data-src="{{ url('storage/'.$dok['dokumen_uploaded_path']) }}" class="btn btn-primary btn-sm previewPdf"><i class="bi bi-eye"></i> Preview</button>
                  </td>
                  <td>{{ $dok['tahun_perolehan'] }}</td>
                  <td><img src="{{ asset('storage/' . $dok['foto_barang_path']) }}" alt="Dokumen Pengadaan" class="img-thumbnail" width="100"></td>
                  <td>
...
<script>
  $(document).ready(function() {
      ...
      $('.select2-add').select2({
          ...
          placeholder: 'Pilih aset (bisa multiple)',
          width: '100%',
          closeOnSelect: false
      }).on('select2:open', function(e) {
          // Add close button to dropdown header
          let dropdown = $('.select2-dropdown');
          if (dropdown.find('.select2-close').length === 0) {
              dropdown.prepend(`<button class="btn btn-sm btn-danger float-end select2-close" id="select2_close" style="margin: 2px 5px;">
                  <i class="bi bi-x-lg"></i>
              </button>`);
              $('.select2-close').on('click', function () {
                  $('.select2-add').select2('close');
              });
          }
      });
  });
